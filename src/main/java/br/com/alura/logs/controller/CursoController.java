...
public class CursoController {
	...
	public ResponseEntity<Page<CursoModel>> getAllCursos(@PageableDefault(page = 0, size = 10, sort = "dataInscricao", direction = Sort.Direction.ASC) Pageable pageable) {
        log.info("Buscando por todos os registros");
	...
	public ResponseEntity<Object> getOneCursos(@PathVariable(value="id") UUID id) {
        ...
        return cursoModelOptional.<ResponseEntity<Object>>map(cursoModel -> ResponseEntity.status(HttpStatus.OK).body(cursoModel))
                .orElseGet(() -> ResponseEntity.status(HttpStatus.NOT_FOUND).body("Curso não encontrado!"));
    }
